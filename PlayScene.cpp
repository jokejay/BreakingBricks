...
void PlayScene::Update(float deltaTime) {
	...
	for (int i = 0; i < SpeedMult; i++) {
		...
		case 3:
			...
			break;
		case 4:
			EnemyGroup->AddNewObject(enemy = new StrongTankEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
			break;
...
void PlayScene::UIBtnClicked(int id) {
	...
	else if (id == 3 && money >= LightGunTurret::Price)
		preview = new LightGunTurret(0, 0);
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
	...
	while (!que.empty()) {
		...
		if (p.y > 0) {
			if (mapState[p.y - 1][p.x] == TILE_DIRT) {
				if (map[p.y - 1][p.x] == -1) {
		...
		if (p.x > 0) {
			if (mapState[p.y][p.x-1] == TILE_DIRT) {
				if (map[p.y][p.x-1] == -1) {
