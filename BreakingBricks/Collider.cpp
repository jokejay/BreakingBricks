...
namespace Engine {
	...
	bool Collider::IsCircleOverlapRect(Point c1, float r1, Point rectp1, Point rectp2) {
		//TBD: determine whether a circle overlap rectangle
		float testx = c1.x;
		float testy = c1.y;
		float rw = rectp2.x - rectp1.x;
		float rh = rectp2.y - rectp2.y;
		float ry = (rectp2.y + rectp2.y) / 2;
		float rx = (rectp2.x + rectp2.x) / 2;
