...
namespace Engine {
	ImageButton::ImageButton(std::string img, std::string imgIn, float x, float y, float w, float h, float anchorX, float anchorY, std::string sound) :
	...
	void ImageButton::OnMouseMove(int mx, int my) {
		...
		if (!mouseIn || !Enabled) {
			...
			if(bmp == imgOut && !HoverSound.empty())
				AudioHelper::PlayAudio(HoverSound);
			bmp = imgIn;
		}
	}
}
